...
export default function Report() {
  ...
  if (!reportData) return <div className="p-12 text-center font-bold text-2xl uppercase">{t('report.loading', 'Loading Report...')}</div>;

  return (
    <div className="min-h-screen bg-background p-8">
      <Card className="max-w-4xl mx-auto bg-white border-4 border-black shadow-bauhaus-md p-8">
        <h1 className="text-5xl font-black uppercase mb-6 border-b-4 border-black pb-4">{t('report.title', 'Full Market Report')}</h1>
        <div className="space-y-6 text-xl">
          <p><strong>{t('report.primary_crop', 'Primary Crop:')}</strong> <span className="uppercase">{t(`settings.${reportData.crop.toLowerCase().replace(/ \/ .*/, '').replace(/ /g, '_')}`, reportData.crop)}</span></p>
          ...
          <div className="mt-8 border-t-4 border-black pt-6">
            <h2 className="text-3xl font-black mb-4">{t('report.historical_trends', 'Historical Trends')}</h2>
